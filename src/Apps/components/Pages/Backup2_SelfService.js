...
import {RedirectTo, LoadingEnd, LoadingStart, setGlobalData, setNewHistory, setScreenshotFile} from "../../actions/ActionApp";
...
class SelfService extends Component{
    ...
    //State declaration...
        state = {
            ...
            alreadyLoaded: false,
            is_everythingLoaded: false
        };

        old_box_temp_val = false;
    ...
    //Input Events...
        ...
        onBlurWallA = ()=>{
            ...
            if(val > 360){
                ...
                val = 44;
                this.props.dispatch(sendNotification("Wall-A should be greater or equals 44cm"));
            }
        ...
        onBlurWallB = ()=>{
            ...
            if(val > 360){
                ...
                val = 44;
                this.props.dispatch(sendNotification("Wall-B should be greater or equals 44cm"));
            }
    ...
    //Basic Functions...
        ...
        quickSetup = () => {
            //this.props.dispatch(LoadingStart('Submit Contact Data...'));
                ...
                if(this.state.do_RefreshScreenLengths) {
                    ...
                    let err = this.checkForBlankBoxes();
                    if(err){
                        console.error({err});
                        this.props.dispatch(sendNotification('Oops! '+err, -1));
        ...
        onChangeWallACalculateC1AndC2 = (wall_a) =>{
            let available_space_for_c1_and_c2 = wall_a - 44;
        ...
        onChangeWallC1CalculateC1AndC2 = (int_WallC1) =>{
            ...
            let available_space_for_c1_and_c2 = int_WallA - 44;
            if(
                int_WallC1 < 0 ||
                int_WallC2 < 0 ||
    ...
    //Move Functions...
        ...
        checkForBlankBoxes = () => {
            let isInvalid = false;
            let isInvalid_BoxA = !(this.state.boxes && this.state.boxes.a);
            let isInvalid_BoxB = !(this.state.boxes && this.state.boxes.b);
            if(this.state.totalBoxes == 1){
                if(isInvalid_BoxA){
                    isInvalid = ("Box-A (of Single Box) not available for this container.");
                }
            }else if(this.state.totalBoxes == 2){
                if(isInvalid_BoxA && isInvalid_BoxB){
                    isInvalid = ("Box-A & Box-B (of Double Boxes) both not available for this container.");
                }else if(isInvalid_BoxA){
                    isInvalid = ("Box-A (of Double Boxes) not available for this container.");
                }else if(isInvalid_BoxB){
                    isInvalid = ("Box-B (of Double Boxes) not available for this container.");
                }
            }
            return isInvalid;
        };
                        ...
                        <div className="container-fluid omg-parts">
                            <div className="container-fluid omg-part omg-part-1">
                                <div className="omg-area">
                                    <input type="number" onChange={this.onChangeWallA} onBlur={this.onBlurWallA} min="44" max="360" className="form-control" id="lengthOfWallA" placeholder="Length of wall A" value={this.state.input_wall_a}/>
                                    <input type="number" onChange={this.onChangeWallB} onBlur={this.onBlurWallB} min="44" max="360" className="form-control" id="lengthOfWallB" placeholder="Length of wall B" value={this.state.input_wall_b}/>
        ...
        changeBoxSize = (box_label,selectedBox)=>{
            ...
            switch(box_label){
                case "a":
                    if(this.checkBoxA_fitInContainer(refine_box_data)){
                        stateBoxes.a = refine_box_data;
                    }
                    break;
                case "b":
                    if(this.checkBoxB_fitInContainer(refine_box_data)){
        ...
        checkBoxA_fitInContainer(newBoxSize){
            console.log("checkBoxA_fitInContainer => ",newBoxSize);

            let box_a = this.state.boxes.a;
            let box_b = this.state.boxes.b;
            let wall_a = this.state.wall_a;
            let wall_b = this.state.wall_b;

            let is_contain_width = (box_b.width + newBoxSize.height) < (wall_a - 10);
            let is_contain_height = (newBoxSize.width) < (wall_b - 10);

            let is_contain = is_contain_width && is_contain_height;

            if(!is_contain){
                if(!is_contain_width){
                    this.props.dispatch(sendNotification(newBoxSize.height+" cms width is not applicaple for Box-A.",-1));
                }else if(!is_contain_height){
                    this.props.dispatch(sendNotification(newBoxSize.width+" cms height is not applicaple for Box-A.",-1));
                }
            }
        ...
        checkBoxB_fitInContainer(newBoxSize){
            ...
            let is_contain_width = (box_a.height + newBoxSize.width) < wall_a;
            let is_contain_height = (newBoxSize.height) < wall_b;

            let is_contain = is_contain_width && is_contain_height;

            if(!is_contain){
                if(!is_contain_width){
                    this.props.dispatch(sendNotification(newBoxSize.width+" cms width is not applicaple for Box-B.",-1));
                }else if(!is_contain_height){
                    this.props.dispatch(sendNotification(newBoxSize.height+" cms height is not applicaple for Box-B.",-1));
                }
            }
